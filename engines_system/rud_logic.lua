...
local reverse_table = {{ -10000, 0.04 }, -- BUGS workaround
	...
	-- Corrected against real M reference: real flight idle at 7,000ft is 65%,
	-- this formula's original constant (44.647) only produced ~55% there (donor
	-- has no equivalent idle formula to compare against -- B's own custom
	-- addition -- so this is anchored to the stated real-world reference point,
	-- offset +9.95 applied to the constant term, curve shape/altitude sensitivity
	-- otherwise unchanged).
	min_idle=math.max(53.5,-2.2412587413e-01*math.pow(alt_baro/1000,2) + 5.3544289044e+00*alt_baro/1000 + 5.4596067567e+01+mid_idle_isa_corr)
