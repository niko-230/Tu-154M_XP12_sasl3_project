...
-- ADDED (2026-08-19): station 3 ("kitch and crew", real capacity 2400kg) had NO property anywhere
-- in this file -- neither read nor written. Whatever weight X-Plane assigned it by default was
-- invisible to the EFB and uncontrollable by the user, a real hidden-CG-shift risk. Forced to 0
-- every frame below (see the always-live weight block) so it can never silently carry weight --
-- not wired into calc_CG's total since 0 contributes nothing regardless of arm. If this station
...
--]]	
	...
	-- REVERTED (2026-08-19): this whole block was gated behind "Fast Load button pressed AND
	-- on ground" -- meaning EFB slider changes (fuel, pax, cargo) NEVER reached the real physics
	-- weight (m_stations/fuel_q) at any other time, including mid-flight. Confirmed by user: fuel
	-- changed mid-flight produced zero behavioral change. Real M applies EFB changes live. Made
	-- unconditional so every slider change reaches physics immediately, matching M's real behavior.
	do
		...
		set(kitch_crew_load,0) -- station 3 forced empty, see note above -- no source of weight for it exists yet
		--set(CG_load, (zfw_cg - 25) * 5.28 / 100)
